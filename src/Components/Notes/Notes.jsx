...
import { auth, database, db, onValue, push, set } from '../../firebase';
import { ref } from 'firebase/storage';
import { addDoc, arrayUnion, collection, doc, getDoc, getDocs, getFirestore, limit, orderBy, query, setDoc, updateDoc } from 'firebase/firestore';
...
const auth = getAuth();
    ...
    // const [tabs, setTabs] = useState([{ id: 1, name: 'Note 1', content: '' }]);
    // const [activeTab, setActiveTab] = useState(1);
    // const [newTabName, setNewTabName] = useState(''); // حالة لإضافة اسم للتبويب الجديد
    // const [userId, setUserId] = useState('user123'); // معرف المستخدم
    ...
    //   setActiveTab(id);
    //   setNoteContent(tabs.find((tab) => tab.id === id)?.content || '');
    // };
    ...
    //   setNewTabName(''); // إعادة تعيين حقل الاسم بعد إضافة التبويب
    // };
    ...
    //         setTabs(userNotes.tabs || []); // Set tabs from Firestore
    //         setActiveTab(userNotes.activeTab || null);
    //         setNoteContent(userNotes.noteContent || ''); // Set default content if available
    //       } else {
    //         // Initialize a new notes structure for the user if no data exists
      ...
      const fetchNotes = async () => {
        ...
        if (docSnap.exists()) {
          ...
          if (fetchedTabs.length === 0) {
            ...
            await updateUserNotes({ tabs: fetchedTabs, activeTab: 1, noteContent: '' }); // Save default tab to Firestore
          }
          setTabs(fetchedTabs);
